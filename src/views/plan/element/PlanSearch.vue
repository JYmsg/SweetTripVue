  ...
  <div>
    ...
    <div class="searchlist p-2 pb-0 row justify-content-center no-gutters">
      <div class="col-lg-8 col-sm-8 m-1">
        <base-input placeholder="Search" v-model="search" addon-left-icon="ni ni-zoom-split-in"> </base-input>
      </div>
...
export default {
  ...
  methods: {
    async detail(index) {
      ...
      await http.get("/placeapi/place/one/" + this.places[index].content_id).then(({ data }) => {
        this.modalPlace = {
    ...
    methods: {
      ...
      async searchAll() {
        ...
        if (this.type == null) {
          ...
          if (this.gugun == null) {
            http
              .get("/placeapi/place/list/sido/" + this.area)
              .then(({ data }) => {
                for (let i = 0; i < data.length; i++) {
            ...
            http
              .get("/placeapi/place/list/sido/" + this.area + "/" + this.gugun)
              .then(({ data }) => {
                for (let i = 0; i < data.length; i++) {
                  this.$set(this.places, i, data[i]);
                }
          ...
          if (this.gugun == null) {
            http
              .get("/placeapi/place/list/" + this.area + "/" + this.type)
              .then(({ data }) => {
                for (let i = 0; i < data.length; i++) {
                  this.$set(this.places, i, data[i]);
                }
            ...
            //전부 선택+키워드 없음.
            http
              .post("/placeapi/place/list/" + this.loginUser.id, {
                sido_code: this.area,
